...
export function makeFlag(scene, x, y, team, { scale = 1, poleH = 90, clothW = 90, clothH = 60 } = {}) {
  ...
  const clothX = clothW / 2 + 2;
  const clothY = -poleH + clothH / 2 + 4;

  let cloth;
  if (scene.textures.exists(flagKey(team.iso))) {
    cloth = scene.add.image(clothX, clothY, flagKey(team.iso)).setDisplaySize(clothW, clothH);
  } else {
    cloth = scene.add.rectangle(clothX, clothY, clothW, clothH, primary).setStrokeStyle(2, secondary);
    const code = scene.add.text(clothX, clothY, team.code, {
      fontSize: '22px', fontFamily: 'Impact', color: '#ffffff', stroke: '#000', strokeThickness: 3,
    }).setOrigin(0.5);
    c.add(code);
    c._code = code;
  }
  ...
  // Gentle wave.
  scene.tweens.add({
    targets: cloth, scaleX: { from: 1, to: 0.85 }, duration: 700 + Math.random() * 300,
